...
def register(container):
  @bp.route('/receive_prompt', methods=['POST'])
  def _receive_prompt():
  ...
  @bp.route('/generate_audio_file', methods=['POST'])
  ...
  @bp.route('/speak_text', methods=['POST'])
  def _speak_text():
    data = request.get_json()
    text = data['text']
    ...
    try:
      ...
      (output_filename, subtitles) = container.azure.gen_audio_file_and_subtitles(edited)
      container.ws.send(json.dumps({ 'edited': edited, 'subtitles': subtitles }))
  ...
  @bp.route('/react_to_screen', methods=['POST'])
  ...
  @bp.route('/erase_memory', methods=['POST'])
  def _erase_memory():
    data = request.get_json()

    try:
      container.llm_short_term_memory.erase_memory()
      container.is_busy = False
      print(f'[ROUTES] is_busy -> {container.is_busy}')
    except Exception as e:
      log_error(e, '/erase_memory')

    return {}

  @bp.route('/cancel_speech', methods=['POST'])
  def _cancel_speech():
    data = request.get_json()

    try:
      container.azure.tts_green_light = False
      sleep(0.25);
      container.azure.tts_green_light = True
    except Exception as e:
      log_error(e, '/cancel_speech')

    return {}

  @bp.route('/sing', methods=['POST'])
  def _sing():
    data = request.get_json()
    song = data['song']
  ...
  # note to self: this method should probably not exist. use for testing ONLY
  @bp.route('/set_backend_state_variable', methods=['POST'])
  def _set_backend_state_variable():
    ...
    try:
      if hasattr(container, name):
        ...
        print(f'[ROUTES] container.{name} -> {getattr(container, name)}')
      else:
        print(f'[ROUTES] No such property: container.{name}')
    except Exception as e:
      log_error(e, '/set_backend_state_variable')

    return {}

  @bp.route('/shut_down_server', methods=['POST'])
  def _shut_down_server():
    data = request.get_json()

    try:
      terminate_pytwitchapi(container)
  ...
  @bp.route('/get_db_rows_by_page', methods=['POST'])
  ...
  @bp.route('/print_raffle_entries', methods=['POST'])
  def _print_raffle_entries():
    data = request.get_json()

    return {
      'entries': list(container.raffle_entries_set)
    }

  @bp.route('/toggle_is_speaking_fast', methods=['POST'])
  def _toggle_is_speaking_fast():
    data = request.get_json()
    container.azure.is_speaking_fast = not container.azure.is_speaking_fast

    return {}

  @bp.route('/process_luna_wheel_queue', methods=['POST'])
  def _process_luna_wheel_queue():
    data = request.get_json()
    if len(container.luna_wheel_queue):
      del container.luna_wheel_queue[0]
    if len(container.luna_wheel_queue):
      container.ws.send(json.dumps({
        'luna_wheel': {
          'user_input': container.luna_wheel_queue[0]
        }
      }))
